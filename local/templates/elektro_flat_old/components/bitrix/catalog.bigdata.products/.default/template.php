				...
				if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):
					if($arSetting["OFFERS_VIEW"] == "LIST"):
						foreach($arItem["OFFERS"] as $key => $arOffer):?>
							$("#catalog-offer-item-bigdata-<?=$arOffer['ID']?> .catalog-item-prop").clone().appendTo("#catalog-offer-item-bigdata-<?=$arOffer['ID']?> .catalog-item-props-mob");
						<?endforeach;
					endif;
				endif;
			endforeach;?>
			...
			<div class="catalog-item-cards">
				<?foreach($arResult["ITEMS"] as $key => $arItem):
					...
					<div class="catalog-item-card<?=$class?>">
						<div class="catalog-item-info">
							...
							<?endif;?>
							<div class="item-price-cont<?=(!in_array('OLD_PRICE', $arSetting['PRODUCT_TABLE_VIEW']) && !in_array('PERCENT_PRICE', $arSetting['PRODUCT_TABLE_VIEW']) ? ' one' : '').((in_array('OLD_PRICE', $arSetting['PRODUCT_TABLE_VIEW']) && !in_array('PERCENT_PRICE', $arSetting['PRODUCT_TABLE_VIEW'])) || (!in_array('OLD_PRICE', $arSetting['PRODUCT_TABLE_VIEW']) && in_array('PERCENT_PRICE', $arSetting['PRODUCT_TABLE_VIEW'])) ? ' two' : '').($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"]) ? ' reference' : '');?>">									
								<?/***PRICE***/
								if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):
										...
										<div class="item-price">
											...
											<?if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])):?>
												<span class="catalog-item-price-reference">
													<?=CCurrencyLang::CurrencyFormat($arItem["TOTAL_OFFERS"]["MIN_PRICE"]["DISCOUNT_VALUE"] * $arSetting["REFERENCE_PRICE_COEF"], $arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CURRENCY"], true);?>
												</span>
											<?endif;?>
										</div>									
									<?endif;
											...
											if($arPrice["CAN_ACCESS"]):
												...
												<?else:?>													
													<div class="item-price">
														...
														<?if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])):?>
															<span class="catalog-item-price-reference">
																<?=CCurrencyLang::CurrencyFormat($arPrice["DISCOUNT_VALUE"] * $arSetting["REFERENCE_PRICE_COEF"], $arPrice["CURRENCY"], true);?>
															</span>
														<?endif;?>
													</div>												
												<?endif;
									...
									if($showBar == true):?>
										<div class="item_time_buy_cont">
											<div class="item_time_buy">
												...
												<script type="text/javascript">												
													$(function() {														
														$("#time_buy_timer_bigdata_<?=$arItem['ID']?>").countdown({
												...
												<div class="time_buy_cont">
													...
													<div class="time_buy_timer" id="time_buy_timer_bigdata_<?=$arItem['ID']?>"></div>
												</div>
											</div>
										</div>
									<?endif;
							...
							<div class="buy_more">
								...
								if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):?>
										...
										if($arItem["TOTAL_OFFERS"]["QUANTITY"] > 0 || $arItem["CATALOG_QUANTITY_TRACE"] == "N"):?>
											<div class="avl">
												...
												<span>
													...
													if($arItem["CATALOG_QUANTITY_TRACE"] == "Y"):
														if(in_array("PRODUCT_QUANTITY", $arSetting["GENERAL_SETTINGS"])):
									...
									<div class="add2basket_block">
										<form action="<?=$APPLICATION->GetCurPage()?>" id="add2basket_bigdata_offer_form_<?=$arItem['ID']?>">
											<a href="javascript:void(0)" class="minus" onclick="if (BX('quantity_bigdata_<?=$arItem["ID"]?>').value > <?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CATALOG_MEASURE_RATIO"]?>) BX('quantity_bigdata_<?=$arItem["ID"]?>').value = parseFloat(BX('quantity_bigdata_<?=$arItem["ID"]?>').value)-<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CATALOG_MEASURE_RATIO"]?>;"><span>-</span></a>
											<input type="text" id="quantity_bigdata_<?=$arItem['ID']?>" name="quantity" class="quantity" value="<?=$arItem['TOTAL_OFFERS']['MIN_PRICE']['CATALOG_MEASURE_RATIO']?>"/>
											<a href="javascript:void(0)" class="plus" onclick="BX('quantity_bigdata_<?=$arItem["ID"]?>').value = parseFloat(BX('quantity_bigdata_<?=$arItem["ID"]?>').value)+<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CATALOG_MEASURE_RATIO"]?>;"><span>+</span></a>
								...
								if($arParams["DISPLAY_COMPARE"]=="Y"):?>
									<div class="compare">
										<a href="javascript:void(0)" class="catalog-item-compare" id="catalog_add2compare_link_bigdata_<?=$arItem['ID']?>" onclick="return addToCompare('<?=$arItem["COMPARE_URL"]?>', 'catalog_add2compare_link_bigdata_<?=$arItem["ID"]?>', '<?=SITE_DIR?>');" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_COMPARE')?>" rel="nofollow"><i class="fa fa-bar-chart"></i><i class="fa fa-check"></i></a>
									</div>
								<?endif;
								/***DELAY***/
								if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):
									if($arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CAN_BUY"]):
										if($arItem["TOTAL_OFFERS"]["MIN_PRICE"]["DISCOUNT_VALUE"] > 0):
											...
											<div class="delay">
												<a href="javascript:void(0)" id="catalog-item-delay-bigdata-min-<?=$arItem['TOTAL_OFFERS']['MIN_PRICE']['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', '<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CATALOG_MEASURE_RATIO"]?>', '<?=$props?>', '', 'catalog-item-delay-bigdata-min-<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', '<?=SITE_DIR?>'<?=(isset($arParams["SHOW_POPUP"]) && $arParams["SHOW_POPUP"] == "N") ? ", true" : ""?>)" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
											</div>
										<?endif;
									endif;
								else:
										...
										foreach($arItem["PRICES"] as $code=>$arPrice):
											if($arPrice["MIN_PRICE"] == "Y"):
												if($arPrice["DISCOUNT_VALUE"] > 0):
													...
													<div class="delay">
														<a href="javascript:void(0)" id="catalog-item-delay-bigdata-<?=$arItem['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arItem["ID"]?>', '<?=$arItem["CATALOG_MEASURE_RATIO"]?>', '<?=$props?>', '', 'catalog-item-delay-bigdata-<?=$arItem["ID"]?>', '<?=SITE_DIR?>'<?=(isset($arParams["SHOW_POPUP"]) && $arParams["SHOW_POPUP"] == "N") ? ", true" : ""?>)" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
													</div>
												<?endif;
											endif;
										endforeach;
				...
				foreach($arResult["ITEMS"] as $key => $arElement):
					if((isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) || (isset($arElement["SELECT_PROPS"]) && !empty($arElement["SELECT_PROPS"]))):
						...
						<div class="pop-up more_options<?=(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) ? (($arSetting['OFFERS_VIEW'] == 'LIST') ? ' offers-list' : '') : '';?>" id="<?=$arItemIDs['ID']?>">
							...
							<div class="item_info">
								<div class="item_image" id="<?=$arItemIDs['PICT']?>">
									...
									if(!isset($arElement["OFFERS"]) || empty($arElement["OFFERS"]) || $arSetting["OFFERS_VIEW"] == "LIST"):?>
										<div class="img">
								...
								<div class="item_block<?=($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"]) ? ' reference' : '').(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"]) ? ($arSetting['OFFERS_VIEW'] == 'LIST' ? ' offers-list' : '') : '');?>">
									<?/***OFFERS_PROPS***/
									if($arSetting["OFFERS_VIEW"] != "LIST"):
												...
												<div class="offers-items">
													...
													<div class="tbody">
														<?foreach($arElement["OFFERS"] as $keyOffer => $arOffer):
															...
															<div class="catalog-item" id="catalog-offer-item-bigdata-<?=$arOffer['ID']?>">
																<div class="catalog-item-info">							
																		...
																		<?foreach($arOffer["PRICES"] as $code => $arPrice):
																			if($arPrice["MIN_PRICE"] == "Y"):
																				if($arPrice["CAN_ACCESS"]):
																					...
																					if($price["HIDE_ZERO"] == "Y"):
																						if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])):
																							if(round($arPrice["DISCOUNT_VALUE"] * $arSetting["REFERENCE_PRICE_COEF"], $price["DECIMALS"]) == round($arPrice["DISCOUNT_VALUE"] * $arSetting["REFERENCE_PRICE_COEF"], 0)):
																					...
																					<?else:?>
																						<span class="catalog-item-price">
																							...
																							<?if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])):?>
																								<span class="catalog-item-price-reference">
																									<?=number_format($arPrice["DISCOUNT_VALUE"] * $arSetting["REFERENCE_PRICE_COEF"], $price["REFERENCE_DECIMALS"], $price["DEC_POINT"], $price["THOUSANDS_SEP"]);?>
																									<span><?=$currency?></span>
																								</span>
																							<?endif;?>
																						</span>
																	...
																	<div class="buy_more">
																		...
																		<?if($arOffer["CAN_BUY"]):
																			if($arOffer["ASK_PRICE"]):?>
																				<a class="btn_buy apuo" id="ask_price_anch_bigdata_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-comment-o"></i><span class="short"><?=GetMessage("CATALOG_ELEMENT_ASK_PRICE_SHORT")?></span></a>
																				<?$properties = false;
																				foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
																					$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
																				}
																				...
																				<div class="add2basket_block">
																					<?foreach($arOffer["PRICES"] as $code => $arPrice):
																						if($arPrice["MIN_PRICE"] == "Y"):
																							...
																							$props = !empty($props) ? strtr(base64_encode(addslashes(gzcompress(serialize($props),9))), '+/=', '-_,') : "";?>
																							<div class="delay">
																								<a href="javascript:void(0)" id="catalog-item-delay-bigdata-<?=$arOffer['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arOffer["ID"]?>', '<?=$arOffer["CATALOG_MEASURE_RATIO"]?>', '<?=$props?>', '', 'catalog-item-delay-bigdata-<?=$arOffer["ID"]?>', '<?=SITE_DIR?>'<?=(isset($arParams["SHOW_POPUP"]) && $arParams["SHOW_POPUP"] == "N") ? ", true" : ""?>)" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
																							</div>
																						<?endif;
																					endforeach;?>
																					<form action="<?=SITE_DIR?>ajax/add2basket.php" id="add2basket_form_bigdata_<?=$arOffer['ID']?>" class="add2basket_bigdata_form">
																						<div class="qnt_cont">
																							<a href="javascript:void(0)" class="minus" onclick="if (BX('quantity_bigdata_<?=$arOffer["ID"]?>').value > <?=$arOffer["CATALOG_MEASURE_RATIO"]?>) BX('quantity_bigdata_<?=$arOffer["ID"]?>').value = parseFloat(BX('quantity_bigdata_<?=$arOffer["ID"]?>').value)-<?=$arOffer["CATALOG_MEASURE_RATIO"]?>;"><span>-</span></a>
																							<input type="text" id="quantity_bigdata_<?=$arOffer['ID']?>" name="quantity" class="quantity" value="<?=$arOffer['CATALOG_MEASURE_RATIO']?>"/>
																							<a href="javascript:void(0)" class="plus" onclick="BX('quantity_bigdata_<?=$arOffer["ID"]?>').value = parseFloat(BX('quantity_bigdata_<?=$arOffer["ID"]?>').value)+<?=$arOffer["CATALOG_MEASURE_RATIO"]?>;"><span>+</span></a>
																					...
																					<button name="boc_anch" id="boc_anch_bigdata_<?=$arOffer['ID']?>" class="btn_buy boc_anch" value="<?=GetMessage('CATALOG_ELEMENT_BOC')?>"><i class="fa fa-bolt"></i><?=GetMessage('CATALOG_ELEMENT_BOC_SHORT')?></button>
																			...
																			<?endif;
																		elseif(!$arOffer["CAN_BUY"]):?>
																			<a class="btn_buy apuo" id="order_anch_bigdata_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-clock-o"></i><span class="short"><?=GetMessage("CATALOG_ELEMENT_UNDER_ORDER")?></span></a>
																			<?$properties = false;
																			foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
																				$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
																			}
