function catalogSetConstructDefault(arSetIds, ajax_path, price_currency, lid, site_dir, element_id, detail_img, items_ratio) {
...
catalogSetConstructDefault.prototype.DeleteItem = function(element, item_id) {
	...
	var sumReferencePrice = +BX.firstChild(wrapObj).getAttribute("data-reference-price");
	...
	if(!!setItems && setItems.length > 0) {
		for(i = 0; i < setItems.length; i++) {			
			...
			sumReferencePrice += +setItems[i].getAttribute("data-reference-price");
	...
	BX.ajax.post(
		this.ajax_path,
		{
			...
			sumPrice : sumPrice,
			sumReferencePrice : sumReferencePrice,
		...
		function(result) {
			...
			if(json.formatReferenceSum) {
				BX.findChild(wrapObj, {className:"set-result-price-reference"}, true, false).innerHTML = json.formatReferenceSum;
			}
...
catalogSetConstructDefault.prototype.Add2Basket = function() {
	...
	BX.ajax.post(
		this.ajax_path,
		{
			...
			setSelectProps: !!BX("select_props_" + this.arSetIDs[0].ID) ? BX("select_props_" + this.arSetIDs[0].ID).getAttribute("value") : "",
...
/***POPUP_FUNCTIONS***/
function catalogSetConstructPopup(ItemsCount, Currency, DefaultItemPrice, DefaultItemDiscountPrice, DefaultItemReferencePrice, DefaultItemDiscountDiffPrice, ajaxPath, setIds, lid, site_dir, element_id, items_ratio, detail_img) {
	...
	this.catalogDefaultItemReferencePrice = DefaultItemReferencePrice;
...
catalogSetConstructPopup.prototype.catalogSetDelete = function(element) {
	...
	for(var i = 0, l = this.catalogSetIds.length; i < l; i++) {		
		if(this.catalogSetIds[i].ID == itemID) {
			var keyDel = i;			
		}
	}	
	this.catalogSetIds.splice(keyDel, 1);	
}
...
catalogSetConstructPopup.prototype.catalogSetAdd = function(element, emptyObj) {
	...
	if(emptyObj) {
		...
		var itemID = objName.getAttribute("data-item-id");
		var itemIblockID = objName.getAttribute("data-item-iblock-id");
...
catalogSetConstructPopup.prototype.recountPrices = function() {
	...
	var sumReferencePrice = +this.catalogDefaultItemReferencePrice;
	...
	for(var i=0; i < setObj.length; i++) {
		if(!BX.hasClass(setObj[i], "item_empty")) {
			...
			if(price)
				sumPrice += +price;
			var referencePrice = priceObj.getAttribute("data-reference-price");
			if(referencePrice)
				sumReferencePrice += +referencePrice;
	...
	BX.ajax.post(
		...
		function(result)
		{
			...
			if(json.formatReferenceSum) {				
				BX("set-result-price-reference-"+element_id).innerHTML = json.formatReferenceSum;
			}
			if(json.formatOldSum) {
				BX("set-result-price-old-"+element_id).innerHTML = json.formatOldSum;
				BX("set-result-price-old-"+element_id).style.display = "block";
			} else {
			...
			if(json.formatDiscDiffSum) {
				...
				BX("set-result-price-percent-"+element_id).style.display = "block";
			} else {
...
catalogSetConstructPopup.prototype.Add2Basket = function() {
	...
	BX.ajax.post(
		this.ajaxPath,
		{
			...
			lid: this.lid,			
			setOffersCartProps: BX.message('setOffersCartProps'),			
			setSelectProps: !!BX("select_props_" + this.catalogSetIds[0].ID) ? BX("select_props_" + this.catalogSetIds[0].ID).getAttribute("value") : "",
		},
		function(result) {			
			$.post(site_dir + "ajax/basket_line.php", function(data) {
				refreshCartLine(data);
