	...
	<?if(count($arResult["ITEMS"]["AnDelCanBuy"]) > 0):?>	
							...
							<?if(in_array("QUANTITY", $arParams["COLUMNS_LIST"])):
								...
								<div class="cart-item-quantity">									
									<div style="float:right;" class="buy_more">
										...
										<input type="text" name="QUANTITY_<?=$arBasketItems["ID"]?>" id="QUANTITY_<?=$arBasketItems["ID"]?>" class="quantity" value="<?=$arBasketItems["QUANTITY"]?>" onchange="updateQuantity('QUANTITY_<?=$arBasketItems["ID"]?>', '<?=$arBasketItems["ID"]?>', <?=$ratio?>, <?=$useFloatQuantityJS?>)" />
		...
		<div class="w100p">			
			<?if($arParams["HIDE_COUPON"] != "Y"):?>	
				<div class="cart-coupon" id="cart-coupon">
					<div class="bx_ordercart_coupon">						
						<input type="text" id="coupon" name="COUPON" value="" placeholder="<?=GetMessage("SALE_COUPON_VAL")?>" />
					...
					<?if(!empty($arResult["COUPON_LIST"])) {
						foreach($arResult["COUPON_LIST"] as $oneCoupon) {
							...
							<div class="bx_ordercart_coupon">								
								...
								<span class="close <?=$couponClass;?>" data-coupon="<?=htmlspecialcharsbx($oneCoupon['COUPON']);?>"><i class="fa fa-times"></i></span>
								<div class="clr"></div>
							</div>
						<?}
			...
			<div class="cart-buttons">				
				<button name="boc_anch_cart" id="boc_anch_cart" class="btn_buy boc_anch_cart" value="<?=GetMessage('SALE_BOC')?>"><?=GetMessage('SALE_BOC')?></button>
