						...
						<?if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):						
							if($arElement["TOTAL_OFFERS"]["QUANTITY"] > 0):?>							
								<div class="avl">
									...
									<span><?=GetMessage("CATALOG_ELEMENT_AVAILABLE")?><?=in_array("PRODUCT_QUANTITY", $arSetting["GENERAL_SETTINGS"]["VALUE"]) ? " ".$arElement["TOTAL_OFFERS"]["QUANTITY"] : ""?></span>
								</div>
							<?else:?>
								<div class="not_avl">
						...
						if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
							if($arElement["TOTAL_OFFERS"]["MIN_PRICE"]["CAN_BUY"]):
								if($arElement["TOTAL_OFFERS"]["MIN_PRICE"]["DISCOUNT_VALUE"] > 0):
									...
									<div class="delay">
										<a href="javascript:void(0)" id="catalog-item-delay-min-<?=$arElement['TOTAL_OFFERS']['MIN_PRICE']['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arElement["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', '<?=$arElement["TOTAL_OFFERS"]["MIN_PRICE"]["CATALOG_MEASURE_RATIO"]?>', '<?=$props?>', '', 'catalog-item-delay-min-<?=$arElement["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>')" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
									</div>
								<?endif;
							endif;
						else:
		...
		if((isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) || (isset($arElement["SELECT_PROPS"]) && !empty($arElement["SELECT_PROPS"]))):
			...
			<div class="pop-up more_options<?=(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) ? (($arSetting['OFFERS_VIEW']['VALUE'] == 'LIST') ? ' offers-list' : '') : '';?>" id="<?=$arItemIDs['ID']?>">
				...
				<div class="item_info">
					...
					<div class="item_block<?=(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) ? (($arSetting['OFFERS_VIEW']['VALUE'] == 'LIST') ? ' offers-list' : '') : '';?>">
						<?/***OFFERS_PROPS***/
						if($arSetting["OFFERS_VIEW"]["VALUE"] != "LIST"):
																	...
																	<div class="add2basket_block">
																		<?foreach($arOffer["PRICES"] as $code => $arPrice):
																			if($arPrice["MIN_PRICE"] == "Y"):
																				...
																				$props = strtr(base64_encode(addslashes(gzcompress(serialize($props),9))), '+/=', '-_,');?>
																				<div class="delay">
																					<a href="javascript:void(0)" id="catalog-item-delay-<?=$arOffer['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arOffer["ID"]?>', '<?=$arOffer["CATALOG_MEASURE_RATIO"]?>', '<?=$props?>', '', 'catalog-item-delay-<?=$arOffer["ID"]?>')" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
																				</div>
																			<?endif;
																		endforeach;?>
