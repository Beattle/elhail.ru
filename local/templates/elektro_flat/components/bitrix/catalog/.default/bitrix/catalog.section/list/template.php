							...
							if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
								if($arElement["TOTAL_OFFERS"]["MIN_PRICE"]["CAN_BUY"]):
									if($arElement["TOTAL_OFFERS"]["MIN_PRICE"]["DISCOUNT_VALUE"] > 0):
										...
										<div class="delay">
											<a href="javascript:void(0)" id="catalog-item-delay-min-<?=$arElement['TOTAL_OFFERS']['MIN_PRICE']['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arElement["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', '<?=$arElement["TOTAL_OFFERS"]["MIN_PRICE"]["CATALOG_MEASURE_RATIO"]?>', '<?=$props?>', '', 'catalog-item-delay-min-<?=$arElement["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>')" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i><span><?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?></span></a>
										</div>
									<?endif;
								endif;
							else:
