...
if(!empty($arResult["ITEMS"])) {
	...
	foreach($arResult["ITEMS"] as $key => $arElement) {
		if($arElement["PROPERTIES"]["TOVAR_VYVEDEN_IZ_ASSORTIMENTA"]["VALUE_XML_ID"] == true){
			unset($arResult["ITEMS"][$key]["CAN_BUY"]);
			unset($arResult["ITEMS"][$key]["PRICES"]);
		}
		//CURRENT_DISCOUNT//
		$arPrice = array();
	...
	foreach($arResult["CATALOGS"] as $catalog) {
		...
		if(!empty($sku) && is_array($sku)) {
			...
			if($arSettingBigdata["OFFERS_VIEW"] == "LIST") {
				...
				while($propInfo = $propertyIterator->fetch()) {			
					if(!in_array($propInfo["CODE"], $arParams["OFFER_TREE_PROPS"][$offersCatalogId]))
						continue;			
					$skuPropList[$catalogId][$propInfo["CODE"]] = $propInfo;
					$skuPropList[$catalogId][$propInfo["CODE"]]["VALUES"] = array();
					$skuPropList[$catalogId][$propInfo["CODE"]]["SHOW_MODE"] = "TEXT";
					$skuPropList[$catalogId][$propInfo["CODE"]]["DEFAULT_VALUES"] = array(
						"PICT" => false,
						"NAME" => "-"
					);
				}
			...
			if($arSKUProp["SHOW_MODE"] == "PICT") {		
				...
				while($ob = $res->GetNextElement()) {
					...
					if(!empty($arFields["PROPERTY_HEX_VALUE"]))
						$skuPropList[$iblockId][$key]["VALUES"][$arFields["ID"]]["HEX"] = $arFields["PROPERTY_HEX_VALUE"];
					if($arFields["PROPERTY_PICT_VALUE"] > 0) {
						...
						if($arFile["WIDTH"] > 24 || $arFile["HEIGHT"] > 24) {
							...
							$skuPropList[$iblockId][$key]["VALUES"][$arFields["ID"]]["PICT"] = array(
								"SRC" => $arFileTmp["src"],
								"WIDTH" => $arFileTmp["width"],
								"HEIGHT" => $arFileTmp["height"],
							);
						} else {
							$skuPropList[$iblockId][$key]["VALUES"][$arFields["ID"]]["PICT"] = $arFile;
						}
					}
				}			
			}
	...
	unset($iblockId, $skuProp);
	
	$arResult["SKU_PROPS"] = $skuPropList;
}

$arResult['EPILOG'] = array();

foreach($arResult["ITEMS"] as $key => $arElement):
	$arResult['EPILOG'][$key]["ID"] = $arElement["ID"];
	$arResult['EPILOG'][$key]["NAME"] = $arElement["NAME"];
	$arResult['EPILOG'][$key]["CAN_BUY"] = $arElement["CAN_BUY"];
	$arResult['EPILOG'][$key]["MIN_PRICE"]["DISCOUNT_VALUE"] = $arElement["MIN_PRICE"]["DISCOUNT_VALUE"];
	if(!isset($arElement["OFFERS"]) || (isset($arElement["OFFERS"]) && empty($arElement["OFFERS"]))):
		$arResult['EPILOG'][$key]["OFFERS"] = true;
	else:
		$arResult['EPILOG'][$key]["OFFERS"] = false;
	endif;
endforeach;
