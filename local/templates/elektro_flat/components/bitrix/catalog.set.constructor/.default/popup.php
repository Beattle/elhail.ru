...
$arSetting["REFERENCE_PRICE"] = $_REQUEST["arParams"]["REFERENCE_PRICE"];
$arSetting["REFERENCE_PRICE_COEF"] = $_REQUEST["arParams"]["REFERENCE_PRICE_COEF"];
	...
	<div class="catalog-item-cards first_section">
		<div class="catalog-item-card set_element">
			<div class="catalog-item-info">
				...
				<div class="item-price-cont<?=($arSetting['REFERENCE_PRICE'] == 'Y' && !empty($arSetting['REFERENCE_PRICE_COEF']) ? ' reference' : '');?>">
					<?$price = CCurrencyLang::GetCurrencyFormat($arCurElementInfo["PRICE_CURRENCY"], "ru");
					if(empty($price["THOUSANDS_SEP"])):
						$price["THOUSANDS_SEP"] = " ";
					endif;					
					if($price["HIDE_ZERO"] == "Y"):						
						if(round($arCurElementInfo["PRICE_DISCOUNT_VALUE"], $price["DECIMALS"]) == round($arCurElementInfo["PRICE_DISCOUNT_VALUE"], 0)):
							$price["DECIMALS"] = 0;
						endif;
					endif;
					...
					<div class="item-price">
						<?if($arCurElementInfo["PRICE_DISCOUNT_VALUE"] < $arCurElementInfo["PRICE_VALUE"]):?>
							<span class="catalog-item-price-old">
								<?=$arCurElementInfo["PRICE_PRINT_VALUE"];?>								
							</span>
						<?endif;?>
						...
						<?if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])):?>
							<span class="catalog-item-price-reference">
								<?=CCurrencyLang::CurrencyFormat($arCurElementInfo["PRICE_DISCOUNT_VALUE"] * $arSetting["REFERENCE_PRICE_COEF"], $arCurElementInfo["PRICE_CURRENCY"], true);?>
							</span>
						<?endif;?>
					</div>
				</div>
		...
		<?foreach($arSetElementsDefault as $arItem):?>
			<div class="catalog-item-card set_item">				
				<div class="catalog-item-info">
					...
					<div class="item-all-title" data-item-id="<?=$arItem['ID']?>" data-item-iblock-id="<?=$arItem['IBLOCK_ID']?>">
					...
					<div class="item-price-cont<?=($arSetting['REFERENCE_PRICE'] == 'Y' && !empty($arSetting['REFERENCE_PRICE_COEF']) ? ' reference' : '');?>">						
						...
						<div class="item-price" data-discount-price="<?=$arItem['PRICE_DISCOUNT_VALUE']?>" data-reference-price="<?=($arSetting['REFERENCE_PRICE'] == 'Y' && !empty($arSetting['REFERENCE_PRICE_COEF']) ? $arItem['PRICE_DISCOUNT_VALUE'] * $arSetting['REFERENCE_PRICE_COEF'] : $arItem['PRICE_DISCOUNT_VALUE']);?>" data-price="<?=$arItem['PRICE_VALUE']?>" data-discount-diff-price="<?=$arItem['PRICE_DISCOUNT_DIFFERENCE_VALUE']?>">
							...
							<?if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])):?>
								<span class="catalog-item-price-reference">
									<?=CCurrencyLang::CurrencyFormat($arItem["PRICE_DISCOUNT_VALUE"] * $arSetting["REFERENCE_PRICE_COEF"], $arItem["PRICE_CURRENCY"], true);?>
								</span>
							<?endif;?>
						</div>
					</div>
		...
		<div class="catalog-item-card set_result">
			<div class="catalog-item-info">
				...
				<div class="item-price-cont<?=($arSetting['REFERENCE_PRICE'] == 'Y' && !empty($arSetting['REFERENCE_PRICE_COEF']) ? ' reference' : '');?>">
					...
					if($price["HIDE_ZERO"] == "Y"):						
						if(round($setPriceVal, $price["DECIMALS"]) == round($setPriceVal, 0)):
							$price["DECIMALS"] = 0;
						endif;
					endif;
					$currency = str_replace("#", " ", $price["FORMAT_STRING"]);?>
					...
					<div class="item-price">
						...
						<?if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])):?>
							<span class="catalog-item-price-reference" id="set-result-price-reference-<?=$curElementId?>">
								<?=CCurrencyLang::CurrencyFormat($setPriceVal * $arSetting["REFERENCE_PRICE_COEF"], $setPriceCurr, true);?>
							</span>
						<?endif;?>
					</div>
				</div>
...
<script type="text/javascript">
	var catalogSetPopupObj = new catalogSetConstructPopup(		
		...
		"<?=CUtil::JSEscape($arSetting['REFERENCE_PRICE'] == 'Y' && !empty($arSetting['REFERENCE_PRICE_COEF']) ? $arCurElementInfo['PRICE_DISCOUNT_VALUE'] * $arSetting['REFERENCE_PRICE_COEF'] : $arCurElementInfo['PRICE_DISCOUNT_VALUE'])?>",
