...
<script type="text/javascript">
	...
	$(function() {
		$("#order_anch_<?=$arParams['ELEMENT_ID']?>").click(function(e){
			...
			$(window).resize();
			$("#order_body_<?=$arParams['ELEMENT_ID']?>").fadeIn(300);
			$("#order_<?=$arParams['ELEMENT_ID']?>").fadeIn(300);
		});
		$("#order_close_<?=$arParams['ELEMENT_ID']?>, #order_body_<?=$arParams['ELEMENT_ID']?>").click(function(e){
			e.preventDefault();
			$("#order_body_<?=$arParams['ELEMENT_ID']?>").fadeOut(300);
			$("#order_<?=$arParams['ELEMENT_ID']?>").fadeOut(300);
		});
	});
	//]]>
</script>
...
<div class="pop-up order" id="order_<?=$arParams['ELEMENT_ID']?>">
	...
	<div class="container">
		...
		<form action="<?=$APPLICATION->GetCurPage()?>" class="new_order_form">
			...
			<div class="submit">
				<button onclick="button_order('<?=$sComponentFolder?>', '<?=$arResult["EMAIL_TO"]?>', '<?=$arResult["REQUIRED"]?>', '<?=$arParams['ELEMENT_ID']?>');" type="button" name="send_button" class="btn_buy popdef"><?=GetMessage('MFT_ORDER_BUTTON');?></button>				
			</div>
		</form>
	</div>
</div>