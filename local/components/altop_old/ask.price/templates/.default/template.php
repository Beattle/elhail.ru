...
<script type="text/javascript">
	...
	$(function() {
		$("#ask_price_anch_<?=$arParams['ELEMENT_ID']?>").click(function(e){
			...
			$(window).resize();			
			$("#ask_price_body_<?=$arParams['ELEMENT_ID']?>").fadeIn(300);
			$("#ask_price_<?=$arParams['ELEMENT_ID']?>").fadeIn(300);
		});
		$("#ask_price_close_<?=$arParams['ELEMENT_ID']?>, #ask_price_body_<?=$arParams['ELEMENT_ID']?>").click(function(e){
			e.preventDefault();			
			$("#ask_price_body_<?=$arParams['ELEMENT_ID']?>").fadeOut(300);
			$("#ask_price_<?=$arParams['ELEMENT_ID']?>").fadeOut(300);
		});
	});
	//]]>
</script>
...
<div class="pop-up ask_price" id="ask_price_<?=$arParams['ELEMENT_ID']?>">
	...
	<div class="container">
		...
		<form action="<?=$APPLICATION->GetCurPage()?>" class="new_ask_price_form">
			...
			<div class="submit">
				<button onclick="button_ask_price('<?=$sComponentFolder?>', '<?=$arResult["EMAIL_TO"]?>', '<?=$arResult["REQUIRED"]?>', '<?=$arParams['ELEMENT_ID']?>');" type="button" name="send_button" class="btn_buy popdef"><?=GetMessage('MFT_ASK_PRICE_BUTTON');?></button>				
			</div>
		</form>
	</div>
</div>