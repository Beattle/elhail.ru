		...
		<div class="container">
			...
			<?if($arResult['NON_AUTHORIZED_USER_CAN_COMMENT'] == "Y") {?>
				<form action="<?=$APPLICATION->GetCurPage()?>" id="new_comment_form" class="new_comment_form">
					...
					<div class="submit">
						<button onclick="button_comment(<?=$arParams['OBJECT_ID']?>, '<?=$arParams['OBJECT_NAME']?>', <?=$arParams['COMMENTS_IBLOCK_ID']?>, '<?=$arResult['URL'];?>', '<?=$sComponentFolder?>', '<?=$arResult["USE_CAPTCHA"]?>', '<?=$arParams["PRE_MODERATION"]?>', '<?=$arResult["PROPS"]?>');" type="button" name="send_button" class="btn_buy popdef"><?=GetMessage('ADD_COMMENT_BUTTON');?></button>						
					</div>
				</form>
			<?} else {?>
