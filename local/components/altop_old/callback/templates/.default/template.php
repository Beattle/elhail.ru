...
$frame = $this->createFrame("callback")->begin("");?>	
	<form action="<?=$APPLICATION->GetCurPage()?>" id="new_callback_form" class="new_callback_form">		
		...
		<div class="submit">
			<button onclick="button_callback('<?=$sComponentFolder?>', '<?=$arResult["EMAIL_TO"]?>', '<?=$arResult["REQUIRED"]?>');" type="button" name="send_button" class="btn_buy popdef"><?=GetMessage('MFT_ORDER');?></button>
		</div>
	</form>
<?$frame->end();?>