...
if($_SERVER["HTTP_X_REQUESTED_WITH"] == "XMLHttpRequest" && $_POST["METHOD"] == "boc") {
	...
	if(!$USER->IsAuthorized()):
		echo "<script>$('#boc_captcha_word_".$_POST["ELEMENT_CODE"].$_POST["ELEMENT_ID"]."').attr('value', '');</script>";
	...
	if($return == true) {
		if(!$USER->IsAuthorized()) {
			...
			echo "<script>$('#boc_cImg_".$_POST["ELEMENT_CODE"].$_POST["ELEMENT_ID"]."').attr('src','/bitrix/tools/captcha.php?captcha_sid=".$cCode."');$('#boc_captcha_sid_".$_POST["ELEMENT_CODE"].$_POST["ELEMENT_ID"]."').val('".$cCode."');</script>";
		}
