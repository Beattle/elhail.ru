...
$frame = $this->createFrame("new")->begin("");
	if(count($arResult["ITEMS"]) > 0):
							...
							if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):
								...
								<div class="add2basket_block">
									<form action="<?=$APPLICATION->GetCurPage()?>" class="add2basket_form">
										...
										<button type="button" class="btn_buy" name="add2basket" value="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_CART')?>" onclick="OpenPropsPopupNew('<?=$arItemIDs["ID"]?>', '<?=$arItem["ID"]?>'<?=($arSetting["OFFERS_VIEW"]["VALUE"] == "LIST" ? ", true" : "");?>);"><i class="fa fa-shopping-cart"></i><span><?=GetMessage("CATALOG_ELEMENT_ADD_TO_CART")?></span></button>
									</form>
								</div>									
									...
									<?if($arItem["CAN_BUY"]):
										...
										<?elseif(!$arItem["ASK_PRICE"]):
											...
											<?endif;?>
												...
												<button type="button" class="btn_buy" name="add2basket" value="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_CART')?>"<?=(isset($arItem["SELECT_PROPS"]) && !empty($arItem["SELECT_PROPS"]) ? " onclick=\"OpenPropsPopupNew('".$arItemIDs["ID"]."', '".$arItem["ID"]."')\"" : " id='".$arItemIDs["BTN_BUY"]."'");?>><i class="fa fa-shopping-cart"></i><span><?=GetMessage("CATALOG_ELEMENT_ADD_TO_CART")?></span></button>
											</form>
										<?endif;
									elseif(!$arItem["CAN_BUY"]):?>
							...
							if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):
								if($arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CAN_BUY"]):
									if($arItem["TOTAL_OFFERS"]["MIN_PRICE"]["DISCOUNT_VALUE"] > 0):
										...
										<div class="delay">
											<a href="javascript:void(0)" id="catalog-item-delay-new-min-<?=$arItem['TOTAL_OFFERS']['MIN_PRICE']['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', 'quantity_new_<?=$arItem["ID"]?>', '<?=$props?>', '', 'catalog-item-delay-new-min-<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', '<?=SITE_DIR?>')" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
										</div>
									<?endif;
								endif;
							/***ITEM_DELAY***/
									...
									foreach($arItem["PRICES"] as $code=>$arPrice):
										if($arPrice["MIN_PRICE"] == "Y"):
											if($arPrice["DISCOUNT_VALUE"] > 0):
												...
												<div class="delay">
													<a href="javascript:void(0)" id="catalog-item-delay-new-<?=$arItem['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arItem["ID"]?>', 'quantity_new_<?=$arItem["ID"]?>', '<?=$props?>', '', 'catalog-item-delay-new-<?=$arItem["ID"]?>', '<?=SITE_DIR?>')" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
												</div>
											<?endif;
										endif;
									endforeach;
		...
		<script type="text/javascript">
			...
			function OpenPropsPopupNew(visual_id, element_id, offers_list) {
							...
							if(!BX(visual_id + "_info")) {
								...
								if(!!qntItems && 0 < qntItems.length) {
									for(i = 0; i < qntItems.length; i++) {					
										qntItems[i].value = BX("quantity_new_" + element_id).value;
									}
								}
							}
						}
