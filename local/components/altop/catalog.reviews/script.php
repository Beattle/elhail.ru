...
if(!empty($iblockString))
	$iblock = unserialize(gzuncompress(stripslashes(base64_decode(strtr($iblockString, '-_,', '+/=')))));

$elementString = $request->getPost("ELEMENT_STRING");
if(!empty($elementString))
	$element = unserialize(gzuncompress(stripslashes(base64_decode(strtr($elementString, '-_,', '+/=')))));
